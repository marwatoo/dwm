...
drawbar(Monitor *m)
{
    ...
    for (i = 0; i < LENGTH(tags); i++) {
        /* use pointer instead of snprintf buffer */
        const char *tlabel = (!(occ & 1 << i) && !(m->tagset[m->seltags] & 1 << i))
            ? tags2[i] : tags[i];
        w = TEXTW(tlabel);
    ...
    w = m->ww - tw - stw - x;            /* compute w first for consistent use */
    if (w > bh) {
        if (m->sel) {
            ...
            drw_text(drw, x, 0, title_w, bh, mid, fribidi_text, 0);

            /* fbounds check before drawing floating indicator */
            if (m->sel->isfloating && boxs + boxw <= title_w)
...
manage(Window w, XWindowAttributes *wa)
{
	...
	if (c->isfloating)
		XRaiseWindow(dpy, c->win);
	attach(c);
	/* follow tag: switch view to the client's tag if it's not on the current tag */
...
shifttag(const Arg *arg) {
	...
	/* compute shifted tag mask */
	if (arg->i > 0) /* left circular shift */
		shifted.ui = (c->tags << arg->i) | (c->tags >> (LENGTH(tags) - arg->i));
	else /* right circular shift */
		shifted.ui = (c->tags >> (-arg->i)) | (c->tags << (LENGTH(tags) + arg->i));

	shifted.ui &= TAGMASK;

	/* apply shifted tags to client (vanilla dwm way) */
	c->tags = shifted.ui;

	/* FOLLOW TAG: switch to the tag where the window moved */
	selmon->seltags ^= 1;
	selmon->tagset[selmon->seltags] = shifted.ui;
...
togglebar(const Arg *arg)
{
    ...
    if (showsystray && systray) {
        ...
        /* calculate total width of tray icons including spacing */
        for (i = systray->icons; i; i = i->next)
            w += i->w + 2; // 2 = padding/margin; adjust as needed

        /* resize systray window to exact width */
        XMoveResizeWindow(dpy, systray->win,
                          selmon->wx + selmon->ww - w - sp,
                          (selmon->showbar ?
                              (selmon->topbar ? selmon->by + vp : selmon->mh - bh + vp)
                              : -bh),
                          w, bh);
    }

    /* move / resize main bar with padding */
...
updatesystray(void)
{
    ...
    int x = m->mx + m->mw;
    int sw = TEXTW(stext) - lrpad + systrayspacing;
    int w = 1;
    ...
    if (!systray) {
        ...
        if (XGetSelectionOwner(dpy, netatom[NetSystemTray]) == systray->win) {
            ...
            XSync(dpy, False);            /* sync before cleanup */
            free(systray);
            systray = NULL;
            return;
        }
    ...
    for (w = 0, i = systray->icons; i; i = i->next) {
        ...
        /* validate icon dimensions before use */
        int ih = (i->h > 0 && i->h <= bh) ? i->h : bh;
        int iw = (i->w > 0) ? i->w : bh;
        XMoveResizeWindow(dpy, i->win, i->x, (bh - ih) / 2, iw, ih);
        w += iw;
    ...
    /* map children before parent to avoid flash */
